...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <!-- Transfo1 : TITRE -->
        <xsl:result-document method="text" href="./extraction-q6/titres.txt">
            ...
            <xsl:variable name="compteurTitre" select="count(//titre/p/item/a[8][text()='obj'])" />
            
            <xsl:text> 
Nous avons </xsl:text> <xsl:value-of select="$compteurTitre"/> <xsl:text> items répondant à ce critère. 
            </xsl:text>
        ...
        <!-- Transfo2 : DESC -->    
        <xsl:result-document  method="text" href="./extraction-q6/desc.txt">
            <xsl:text> Extraction (au format TXT) d'items en relation de dépendance syntaxique de type OBJ (les classer et les compter) dans description.</xsl:text> <xsl:text>
            </xsl:text>
            
            <xsl:variable name="compteurDesc" select="count(//description/p/item/a[8][text()='obj'])" />
            
            <xsl:text> 
Nous avons </xsl:text> <xsl:value-of select="$compteurDesc"/> <xsl:text> items répondant à ce critère. 
            </xsl:text>
...
<xsl:template match="file" mode="titre">
    ...
    <xsl:for-each select="./titre/p/item">
        ...
        <xsl:if test="./a[8][text()='obj']">
            ...
            <xsl:variable name="dep" select="./a[4]/text()"/>
...
<xsl:value-of select="preceding-sibling::item[$diffDepGouv]/a[4]/text()"/> 
<xsl:text> - </xsl:text> 
