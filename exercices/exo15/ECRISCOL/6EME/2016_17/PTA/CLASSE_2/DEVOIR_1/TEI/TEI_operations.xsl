	...
	<html>
		<head>
			...
			<style type="text/css">
				...
				h3 { color:red; font: 130% sans-serif }
...
<xsl:template match="annotation"><xsl:apply-templates select="b"/></xsl:template>
...
<xsl:template match="remplacement">
		<span style="background-color:#99ff33"><b>&lt;</b><xsl:apply-templates select="b"/><xsl:text> || </xsl:text><span style="background-color:#66ccff;"><xsl:apply-templates select="a"/></span><b>&gt;</b></span>
